...
class CoDi(DDPM):
    def __init__(self,
        ...
        # Build only essential components to reduce memory when running image-only pipelines
        try:
            self.audioldm = get_model()(audioldm_cfg) if (audioldm_cfg is not None) else None
        except Exception:
            self.audioldm = None
        ...
        try:
            self.optimus = get_model()(optimus_cfg) if (optimus_cfg is not None) else None
        except Exception:
            self.optimus = None
        ...
        try:
            self.clap = get_model()(clap_cfg) if (clap_cfg is not None) else None
        except Exception:
            self.clap = None
