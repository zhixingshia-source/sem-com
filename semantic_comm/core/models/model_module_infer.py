...
class model_module(pl.LightningModule):
    def __init__(self, data_dir='pretrained', pth=["CoDi_encoders.pth"], fp16=False):
        ...
        if fp16:
            net = net.half()
        for path in pth:
            sd = torch.load("/home/liz0g/semantic-communication/semantic_comm/" + data_dir + "/" + path, map_location='cpu')
        ...
        # Lazily create sampler to reduce peak memory
        self.sampler = None
    ...
    def inference(self, xtype=[], condition=[], condition_types=[], n_samples=1, mix_weight={'video': 1, 'audio': 1, 'text': 1, 'image': 1}, image_size=256, ddim_steps=50, scale=7.5, num_frames=8):
        ...
        for i, condition_type in enumerate(condition_types):
            if condition_type == 'image':
                ctemp1 = regularize_image(condition[i]).to(next(net.parameters()).device)
                ctemp1 = ctemp1[None].repeat(n_samples, 1, 1, 1)
                cim = net.clip_encode_vision(ctemp1).to(next(net.parameters()).device)
                uim = None
                if scale != 1.0:
                    dummy = torch.zeros_like(ctemp1).to(next(net.parameters()).device)
                    uim = net.clip_encode_vision(dummy).to(next(net.parameters()).device)
                conditioning.append(torch.cat([uim, cim]))
                
            elif condition_type == 'video':
                ctemp1 = regularize_video(condition[i]).to(next(net.parameters()).device)
                ...
                ctx = net.clip_encode_text(n_samples * [condition[i]]).to(next(net.parameters()).device)
                utx = None
                if scale != 1.0:
                    utx = net.clip_encode_text(n_samples * [""]).to(next(net.parameters()).device)
        ...
        for i, xtype_i in enumerate(xtype):
            z[i] = z[i].to(next(net.parameters()).device)
