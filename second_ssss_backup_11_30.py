...
def encode_text(list_of_str: List[str]) -> torch.Tensor:
    with torch.no_grad():
        ...
        z_seq = z_seq.float().detach()
    return z_seq.to(device)
...
def main():
    ...
    if args.all:
        stems=[st for st in list_stems() if has_modalities(st)>=2]
        assert len(stems)>0, "没有可处理的 stem"
    else:
    ...
    for st in stems:
        ...
        # 8) 保存
        out_dir=Path(args.out); out_dir.mkdir(parents=True, exist_ok=True)
        (out_dir/f"{st}_payload.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2))
        torch.save(reps.rep_vectors.cpu(), str(out_dir/f"{st}_reps.pt"))
        (out_dir/f"{st}_members.json").write_text(json.dumps(reps.rep_members, ensure_ascii=False, indent=2))
        ...
        (out_dir/f"{st}_stats.json").write_text(json.dumps(stats, ensure_ascii=False, indent=2))
