...
def _patch_codi_clip_tokenizer():
    ...
    print("⚙️ 已 patch CoDi CLIPTokenizer.get_vocab（兼容新 transformers）", flush=True)
DATA = ROOT / "data"
...
def _ensure_hf():
    ...
    _HF_READY = True
    print("ℹ️ 使用 HF CLIP tokenizer / image processor 兜底")
...
def encode_text(list_of_str: List[str]) -> torch.Tensor:
    """
    纯 CoDi 文本编码：使用 CoDi 的 CLIPTokenizer + text encoder，
    不再依赖 HF 兜底。返回形状 [B, L, D] 的 token 序列。
    """
    _ensure_codi()
    clip = getattr(net, "clip", None)
    if clip is None:
        raise RuntimeError("CoDi net.clip 不存在，检查 CoDi encoders 是否正确加载。")
    ...
    # 强制 Long，防止 ShortTensor 之类问题
    if input_ids.dtype != torch.long:
        input_ids = input_ids.long()

    # 用 CoDi 里的 text encoder 前向
...
@torch.no_grad()
def encode_audio(paths: List[str], seconds: float = 8.0, n_segs: int = 24) -> Optional[torch.Tensor]:
    """
    多 token audio 编码：
    - 先截取/填充到固定时长 `seconds`
    - 再沿时间均匀切成 n_segs 段
    - 每一段通过 CoDi 的 clap 编码，视为一个 token
    返回形状 [B, S, D] 的 token 序列（S ≈ n_segs），给 SemanticExtractor 抽 K-token。
    """
    _ensure_codi()
    ...
    for p in paths:
        ...
        if not p.exists():
            continue
        w, sr = torchaudio.load(str(p))   # (C, T)
        w = w.mean(0, keepdim=True)       # 单声道 (1, T)
        ...
        if w.size(1) >= L:
            st = random.randint(0, max(1, w.size(1) - L))
            w = w[:, st:st + L]
        else:
    ...
    def _segment_wave(w: torch.Tensor, n_segs: int) -> List[torch.Tensor]:
        """
        把单条 waveform 均匀切成 n_segs 段，每段 padding 成相同长度。
        输入: w: (1, L)
        输出: List[(1, T_seg)]
        """
        if n_segs <= 1:
        ...
        L = w.size(1)
        # 均匀切分边界
        starts = [int(i * L / n_segs) for i in range(n_segs)]
        ends = starts[1:] + [L]
        segs = [w[:, st:ed] for st, ed in zip(starts, ends)]
        max_len = max(s.size(1) for s in segs)
        segs = [F.pad(s, (0, max_len - s.size(1))) if s.size(1) != max_len else s for s in segs]
        return segs
    ...
    # 拼 batch: (总段数, T_seg)
    batch = torch.cat(segments, dim=0).to(device)

    # CoDi CLAP 编码
    out = clap.encode_audio_noproj(batch) if hasattr(clap, "encode_audio_noproj") else clap(batch)

    # out: (总段数, D) 或 (总段数, T', D)，后者做个 mean pooling
    if out.ndim == 3:
        out = out.mean(dim=1)
    elif out.ndim != 2:
        out = out.view(out.shape[0], -1)

    feats_per_seg = out  # (总段数, D)
    ...
    max_c = max(counts)
    for c in counts:
        f = feats_per_seg[idx:idx + c]  # (c, D)
        idx += c
        if c < max_c:
            pad = f.new_zeros(max_c - c, f.size(1))
            f = torch.cat([f, pad], dim=0)
        all_samples.append(f.unsqueeze(0))  # (1, max_c, D)

    Z = torch.cat(all_samples, dim=0)       # (B, S, D)
    return Z.float().detach()
...
def _probe_dim_audio():
    """
    探测 audio token 维度（用 CoDi 的 clap 编码一条 wav）。
    对 Clotho 这种 text+audio 数据集，就是我们对齐的第二个模态。
    """
    try:
        # 在 data_root/audio 下递归找任意一个 .wav
        any_wav = next(iter(DAUD.rglob("*.wav")), None)
        if not any_wav:
            return None
        z = encode_audio([str(any_wav)], seconds=8.0)
        if z is None:
            return None
...
def _encode_once(st: str, take_img=4, aud_seconds=8.0):
    """
    对单个样本 stem 做一次编码：
    - 文本：data_root/text/**/{stem}.txt 或 .srt
    - 图像：data_root/image/{stem}_*.jpg  （如果有的话）
    - 音频：data_root/audio/**/{stem}.wav
    对 Clotho 来说只有 text+audio，image 分支自然会是 None。
    """
    t = i = a = None
    ...
    # ---- audio ----
    wav_path = DAUD / f"{st}.wav"
    if wav_path.exists():
        a = encode_audio([str(wav_path)], seconds=aud_seconds)
        if a is None:
            print(f"(音频编码失败，跳过 audio: {wav_path})")
...
def main():
    ...
    # ==== 只对齐 text + audio（支持 dev / eval 子目录，例如 Clotho） ====

    def collect_text_audio_stems():
        """
        返回一组相对路径的 stem：
        例如 audio/dev/clotho_dev_000001.wav 和 text/dev/clotho_dev_000001.txt
        对应的 stem 都是 "dev/clotho_dev_000001"
        """
        audio_stems = {
        ...
        both = sorted(audio_stems & text_stems)
        return both

    all_stems = collect_text_audio_stems()
    assert all_stems, (
        f"在 {DAUD} / {DTXT} 下没有找到任何 (wav, txt) 成对样本，"
        f"确认 data_root 填对了吗？"
    )
    print(f"样本总数（text+audio 对齐）: {len(all_stems)}", flush=True)
